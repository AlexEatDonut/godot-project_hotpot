...
func _handle_water_physics(delta) -> bool:
	...
	if not is_on_floor():
		velocity.y -= ProjectSettings.get_setting("physics/3d/default_gravity") * 0.2 * delta
...
func negativeToPositive(number):
	if number < 0 :
		number = 0 - number
	return number
...
func player_give_health(health, ratio):
	...
	Playerinfo.increase_health(health_rounded, ratio)
	hud_give_hp()
...
#CODE FOR FOCUS OF THE MOUSE
func _mouse_visible(value):
	if value == true:
		Input.set_mouse_mode(Input.MOUSE_MODE_VISIBLE)
	elif value == false:
		Input.set_mouse_mode(Input.MOUSE_MODE_CAPTURED)
